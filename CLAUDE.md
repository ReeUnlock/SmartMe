...
frontend/src/
  ...
  api/              — client.js (apiFetch + apiUpload z JWT, VITE_API_URL env, 401+403→redirect login), pliki per moduł (auth, calendar, shopping, expenses, plans, voice, receipts, billing, rewards, admin)
  config/
...
```
users               — multi-user auth (username, email, hashed_password, onboarding_completed, plan, is_email_verified, email_verified_at, last_seen_at, voice_calls_total, receipt_scans_total, login_count)
