...
frontend/src/
  ...
  components/
    ...
    auth/           — SetupPage, LoginPage, ForgotPasswordPage, OnboardingPage (3 steps), ProtectedRoute
...
- Parser: 50+ znanych sklepów PL, regexowe daty (DD.MM.YYYY, YYYY-MM-DD), SUMA/RAZEM/DO ZAPŁATY keywords + OCR misread variants, item lines
- **Parser fallbacks**: jeśli keyword total nie znaleziony → suma itemów → largest price on any line
...
- `/api/auth/setup`: 3/min, `/api/auth/login`: 5/min, `/api/auth/change-password`: 5/min, `/api/auth/forgot-password`: 3/min, `/api/feedback`: 5/min
