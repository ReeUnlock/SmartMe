...
frontend/src/
  ...
  components/
    ...
    admin/          — panel admina (ciemny motyw gray.900, osobny od SmartMe UI)
      AdminRoute.jsx          — guard (sessionStorage admin_key check)
      AdminLoginPage.jsx      — formularz klucza API
      AdminLayout.jsx         — sidebar (240px) + topbar + content
      AdminDashboard.jsx      — karty KPI + top costly users + feedback breakdown
      AdminUsers.jsx          — tabela userów z search/filtr/sort/paginacja
      AdminUserDetail.jsx     — szczegóły usera (aktywność, AI koszty, nagrody, ostatnie dane)
...
- **Frontend auth**: klucz w `sessionStorage('admin_key')` — wygasa po zamknięciu karty
- **Motyw**: ciemny (gray.900/gray.800), osobny od pastelowego SmartMe

### User tracking (middleware + countery)
- `last_seen_at` — HTTP middleware (BackgroundTask), aktualizowane przy każdym żądaniu z JWT
...
| Strona | Ścieżka | Opis |
|--------|---------|------|
| Login | `/admin` (bez klucza) | Ciemny formularz, waliduje klucz przez `/api/admin/health` |
| Dashboard | `/admin` | 8 kart KPI, top costly users, feedback breakdown, aktywność 7d/30d |
| Users | `/admin/users` | Tabela z search (debounce 300ms), filtr plan, sort, paginacja |
| User detail | `/admin/users/:id` | Aktywność, AI koszty, nagrody, ostatnie wydatki/wydarzenia, subskrypcja |
