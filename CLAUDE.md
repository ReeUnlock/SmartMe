...
```
backend/app/
  main.py           — FastAPI app, rejestracja routerów, GZip, CORS, health check, last_seen_at middleware, admin key seeding (lifespan)
  config.py         — Settings (env vars, CORS_ORIGINS, FRONTEND_URL, STRIPE_PRICE_3M, STRIPE_PRICE_12M)
  ...
  shopping/         — listy zakupów, kategorie, itemy, toggle, reorder, to-expense (smart category split), active list limit (403)
  ...
  voice/            — transkrypcja (Whisper) + parsowanie intencji (GPT) + executors + calendar_validator + prompts.py + daily voice limit (429)
  receipts/         — OCR paragonów (Tesseract pol), heurystyczny parser (sklep, data, suma), auto-crop OpenCV
  common/           — TimestampMixin, pagination, email.py (Resend transactional emails)
  billing/          — Stripe billing: checkout (price_id selection), portal, webhooks, subscription model, feature limits (enforced), schemas (PricingTier)
  rewards/          — backend persistence for rewards system (models.py, router.py, schemas.py) — GET/PATCH /api/rewards
  admin/            — panel admina: auth (X-Admin-Key header, SHA-256 verification), router, schemas, service (users list, user detail, global stats, feedback list, AI cost estimation)
  notifications/    — (puste — przyszłe push notifications, pywebpush zainstalowany)
  ai/               — (puste — przyszłe AI features)
backend/alembic/    — migracje DB (21 plików, env.py importuje wszystkie modele)
backend/tests/      — testy backendowe (pytest, SQLite in-memory)
  ...
  admin/              — 13 testów admin panel (auth 403, health, stats schema, users list/pagination/search/filter, user detail, feedback list, AI cost)
...
appstore/           — App Store submission materials
  app-store-texts.md  — opisy, keywords, what's new (PL + EN)
  privacy-nutrition-label.md — App Privacy nutrition label declarations
```

## Struktura frontendu
```
frontend/src/
  App.jsx           — Router + ChakraProvider + QueryClientProvider + ErrorBoundary + Global Overlays (lazy) + CookieConsent + LimitReachedModal
  theme.js          — pastelowa paleta: rose, peach, sage, sky, lavender + semantic tokens
  api/              — client.js (apiFetch + apiUpload z JWT, VITE_API_URL env, ApiError class z .status/.body, limit error detection, 401+403→redirect login), pliki per moduł (auth, calendar, shopping, expenses, plans, voice, receipts, billing, rewards, admin)
  config/
  ...
  utils/
    ...
    platform.js — isIOS(), isAndroid(), isNative(), isWeb() detection (Capacitor + userAgent)
  hooks/
    ...
    useShopping.js      — TanStack Query, optimistic toggle, categories (staleTime 5min), shopping limit error → LimitReachedModal
    ...
    useVoiceCommand.js  — zustand, MediaRecorder, startRecording/stopRecording/confirmAction, voice limit error → LimitReachedModal
  ...
  components/
    ...
    common/
      ...
      LimitReachedModal.jsx  — modal limitu Free (zustand store useLimitModal, BottomSheetDialog, CTA → /ustawienia)
      SettingsPage.jsx       — profil, hasło, reset, dźwięki, feedback, avatar link, pomoc (intro tour), polityka prywatności, subskrypcja (web: checkout, iOS: info text)
    ...
    billing/        — PricingCard (plan + tier prop, full price display), PricingSection (Free + Pro 3M + Pro 12M grid, per-tier checkout)
    admin/          — panel admina (ciemny motyw gray.900, osobny od SmartMe UI)
      ...
      utils.js                — estimate_cost_breakdown (mirrors backend pricing)
...
## Baza danych — tabele (18)
```
users               — multi-user auth (username, email, hashed_password, onboarding_completed, plan, is_email_verified, email_verified_at, last_seen_at, voice_calls_total, receipt_scans_total, login_count, voice_calls_today, voice_calls_today_date)
...
- **Migracja `257290e4561e`**: dodaje `users.plan` + tabelę `subscriptions`
- **Migracja `e5f6a7b8c9d0`**: dodaje tabelę `user_rewards` (rewards persistence)
- **Migracja `g2a3b4c5d6e7`**: dodaje `users.voice_calls_today` + `users.voice_calls_today_date` (daily voice limit tracking)
- **Baza produkcyjna**: wymaga `alembic upgrade head` dla billing + voice limits
...
/admin             — panel admina (własny guard, sessionStorage key, ciemny motyw)
/admin/users       — lista użytkowników z filtrowaniem
/admin/users/:id   — szczegóły użytkownika
/admin/feedback    — lista feedbacków
```
...
### Admin API
- Auth: `X-Admin-Key` header (SHA-256 verified against `admin_api_keys` table), returns 403 on invalid key
- Key auto-generated on first startup if table is empty (logged once, never shown again)
- `GET /api/admin/health` — admin health check (status, db_users count, timestamp)
- `GET /api/admin/users` — paginated user list with counts, rewards, subscription, AI cost (params: page, per_page, search, plan, sort_by, sort_dir)
- `GET /api/admin/users/{user_id}` — user detail with recent expenses/events, achievements, avatar
- `GET /api/admin/stats` — global stats (users, activity, feedback breakdown, AI costs, subscriptions)
- `GET /api/admin/feedback` — paginated feedback list (params: page, per_page, category)
- `DELETE /api/admin/users/{user_id}` — delete user and all data (cascade), returns `{deleted, user_id, email}`

### Billing API
- `GET /api/billing/plans` — public, pricing info (Free/Pro features + pricing_tiers: 3M=99zł, 12M=299zł)
- `GET /api/billing/subscription` — current user's subscription status
- `POST /api/billing/checkout` — create Stripe Checkout session (accepts optional `price_id` in body, validated against STRIPE_PRICE_3M/12M/PRO)
...
### Billing — modele subskrypcji
| Plan | Cena | Okres | Stripe env var |
|------|------|-------|---------------|
| Pro 3 miesiące | 99 zł | co 3 miesiące | `STRIPE_PRICE_3M` |
| Pro 12 miesięcy | 299 zł | co rok | `STRIPE_PRICE_12M` |
...
- **Nie parsuje pozycji/itemów** — wyciąga tylko: sklep, data, suma końcowa, sugerowana kategoria
- Max image: 10 MB, formaty: JPEG, PNG, WebP, HEIC/HEIF (+ `application/octet-stream` z magic-byte sniffing)
- **Pipeline**: EXIF transpose → resize (max 2000px) → grayscale → autocontrast → **auto-crop (OpenCV contour detection)** → denoise → adaptive threshold → Tesseract (psm=4, single column)
- **Auto-crop**: OpenCV edge detection + contour finding wykrywa prostokąt paragonu na tle (blat, stół) i wycina go, eliminując szum z tekstury tła. Aktywuje się gdy kontur to 15-90% powierzchni obrazu.
- **Dual-pass OCR**: Pass 1 (adaptive threshold, psm=4) + Pass 2 (bottom 70% crop, light denoise, psm=6) — pass 2 uruchamiany gdy pass 1 nie zawiera keywords SUMA/RAZEM
...
- **Store detection**: kotwiczenie na "PARAGON FISKALNY" — szuka nazwy sklepu w liniach nagłówkowych (przed PARAGON), filtruje garbage OCR (linie z niskim % liter)
- **Date detection**: YYYY-MM-DD (first priority), DD.MM.YYYY, DD.MM.YY — unambiguous format tried first
- **Total detection**: priority-based, first-match-wins for high-priority — SUMA PLN/SUMA:/RAZEM/DO ZAPŁATY (high) → WPŁATA (low, largest-wins) → **payment math (GOTÓWKA - RESZTA)** → largest price (fallback)
- **Payment math fallback**: jeśli SUMA jest nieczytelna, oblicza total z GOTÓWKA - RESZTA lub bierze kwotę KARTA/VISA/MASTERCARD
...
- **Skip patterns**: GOTÓWKA, Sp: (VAT subtotals), Udzielono, #Kasa, Kasjer, łącznie rabat, A=12,92 (VAT lines), product lines (ilość×cena, 7,50C)
...
- Frontend: pojedynczy wydatek (bez listy itemów) — opis (storeName), kwota, data, kategoria, kto płaci
...
### Architektura
- **Auth**: klucz API w headerze `X-Admin-Key`, SHA-256 hash w tabeli `admin_api_keys`, 403 przy błędnym kluczu
- **Seeding**: przy starcie app (lifespan) jeśli brak kluczy → generuje `adm_<64hex>`, loguje raz, nie pokazuje ponownie
- **Frontend auth**: klucz w `sessionStorage('admin_key')` (domyślnie, wygasa po zamknięciu karty) lub `localStorage('admin_key')` z expiry 30 dni ("Zapamiętaj" checkbox)
- **Motyw**: ciemny (gray.900/gray.800), osobny od pastelowego SmartMe

### User tracking (middleware + countery)
- `last_seen_at` — HTTP middleware (BackgroundTask), aktualizowane przy żądaniach z JWT (pomija /health, /admin, /auth/login|register, /billing/webhooks)
- `login_count` — inkrementowany w `/api/auth/login` po sukcesie
- `voice_calls_total` — inkrementowany w `/api/voice/process` po sukcesie
- `receipt_scans_total` — inkrementowany w `/api/receipts/scan` po sukcesie

### AI cost estimation (hardcoded OpenAI pricing, marzec 2026)
- Whisper: $0.006/min × śr. 30s = $0.003/wywołanie
- GPT-4o mini: $0.150/1M input + $0.600/1M output, śr. 800 input + 150 output tokenów
- Receipts (Tesseract): $0 (lokalne OCR)
- Funkcja: `estimate_user_cost(voice_calls, receipt_scans)` w `admin/service.py`

### Frontend — strony admina
| Strona | Ścieżka | Opis |
|--------|---------|------|
...
### Migracje
- `f0a1b2c3d4e5` — `users`: last_seen_at, voice_calls_total, receipt_scans_total, login_count
- `f1b2c3d4e5f6` — tabela `admin_api_keys` (key_hash, label, created_at, last_used_at)
- `g2a3b4c5d6e7` — `users`: voice_calls_today, voice_calls_today_date (daily voice limit)
...
- **Auth**: multi-user, rejestracja z weryfikacją email, token-based password reset (24h ważność). HTTPBearer zwraca 403 gdy brak tokenu — `client.js` traktuje 401+403 jako unauthorized (redirect → /login), ALE 403/429 z `error` field w detail (limit errors) są przepuszczane jako `ApiError` z `.status`/`.body`
...
- **Mutation errors**: globalne → ErrorToast (zustand store, montowany w App.jsx), limit errors (voice/shopping) → LimitReachedModal (skip ErrorToast)
...
- **Testy backend admin**: `docker exec anelka-backend python -m pytest tests/admin/ -v --tb=short` (13 testów, SQLite in-memory)
...
- [x] Faza A: Admin Panel (backend: admin API keys, user tracking, stats/users/feedback endpoints, AI cost estimation; frontend: ciemny motyw, login kluczem, dashboard KPI, tabela userów, user detail, feedback lista)
...
- **Backend admin**: 13 testów pytest (auth 403 ×2, health ×1, stats ×2, users list/pagination/search/filter ×4, user detail ×2, feedback ×1, AI cost ×1)
...
frontend/
  ...
  Dockerfile.prod           — Prod multi-stage (Vite build → nginx:alpine static serve)
  nginx.conf                — Nginx config wewnątrz frontend container (SPA fallback, cache headers)
...
- **iOS**: `NSMicrophoneUsageDescription`, `NSCameraUsageDescription`, `NSPhotoLibraryUsageDescription` (Info.plist, po angielsku — wymóg App Store Review)
...
- **Info.plist descriptions**: po angielsku (Apple wymaga w języku domyślnym app = English)
- **Billing UI na iOS**: ukryte (`isIOS()` guard) — App Store Review guideline 3.1.1 (zakaz linkowania do zewnętrznych płatności)
- **Platform detection**: `frontend/src/utils/platform.js` — `isIOS()`, `isAndroid()`, `isNative()`, `isWeb()`
...
- **Frontend prod**: `Dockerfile.prod` (multi-stage: Vite build → nginx:alpine), `frontend/nginx.conf` (SPA fallback + cache: no-cache index.html, immutable /assets/)
- **Nginx outer**: `nginx/nginx.prod.conf` — SSL termination, proxy do backend + frontend containers, landing static
...
| Frontend | Vite dev server (HMR) | Static build (nginx:alpine w kontenerze) |
| Frontend Dockerfile | `Dockerfile` | `Dockerfile.prod` (multi-stage: build → nginx) |
| Frontend nginx | — | `frontend/nginx.conf` (SPA fallback + cache headers) |
| Outer nginx | `nginx/nginx.conf` | `nginx/nginx.prod.conf` (SSL, proxy, landing) |
...
| Cache | Brak | `index.html` no-cache, `/assets/` immutable 1y |
